...
module_data_path=os.path.join(data_path,'gae_pro
')
if __name__=="__main__":
    sys.path.append(default_path)
    noarch=os.path.abspath(os.path.join(default_path,'lib','noarch'))
...
class CheckIp(front_base.check_ip.CheckIp):
    def check_response(self,response):
        server_type=response.headers.get('Sercer',"")
...
class CheckAllIp(object):
    def __init__(self):
        ...
        openssl_context=SSLContext(logger,ca_certs=ca_certs,cipher_suites=[b'ALL',b"!RC4-SHA",b"!ECDHE-RSA-RC4-SHA",b"!ECDHE-RSA-AES128-GCM-SHA256",b"!AES128-GCM-SHA256",b"!ECDHE-RSA-AES128-SHA",b"!AES128-SHA"])
        ...
        self.out_fd=open(os.path.join(module_data_path,"ipv6_list.txt"),"w")
    def get_ip(self):
        with self.lock:
...
if __name__=="__main__":
    ...
    if len(sys.argv)>3:
        ...
        wait_time=0
    ca_certs=os.path.join(current_path,"cacert.pem")
    openssl_context=SSLContext(logger,ca_certs=cs_certs,protocol="TLSv1_2")
