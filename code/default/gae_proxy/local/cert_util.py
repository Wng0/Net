...
class CertUtil(object):
    ...
    ca_vendor = 'GoAgent' # TODO: here should be oonet
    ...
    def creat_ca():
        ...
        ca.set_subject(subj)
        ca.set_pubkey(key)
        ca.add_extensions([OpenSSL.crypto.X509Extension(b'basicConstraints', False, b'CA:TRUE', subject=ca, issuer=ca)])
    ...
    def generate_cert_keyfile():
        xlog.info("generate cert's key file:%s", CertUtil.cert_keyfile)
    ...
    def _get_cert(commonname, isip = False, sans = None):
        ...
        cert.set_version(2)
        # setting the only serial number, the brower will refused fixed serial number when cert update.
        ...
        cert.gmtime_adj_notAfter(CertUtil.ca_subject)
        ...
        certfile = os.path.join(CertUtil.ca_certdir, util.to_str(commonname) + 'crt')
    ...
    def import_windows_ca(certfile):
        ...
        class CERT_CONTEXT(ctypes.Structure):
            _fields_ = [
                ...
                ('pbCertEndcoded', ctypes.POINTER(ctypes.wintypes.BYTE)),
        ...
        for store in (CERT_SYSTEM_STORE_LOCAL_MACHINE, CERT_SYSTEM_STORE_CURRENT_USER):
            try:
                ...
                xlog.warning('CertUtil.import_window_ca failed: %r', e)
        ...
        if ret == 0 and __name__ != "__main__":
            ...
            CertUtil.wind32_notify(msg = ' 已经导入GoAgent证书，请重启浏览器。', title = 'Restart browser need.')
    ...
    def import_linux_ca(common_name, ca_file):
        def get_linux_ca_sha1(nss_path):
            commonname = "GoAgent XX-net - GoAgent" # TODO: here should be GoAgent - net
        ...
        xlog.info("Removeing old cert in database #HOME/.pki/nssdb")
        cmd_line = 'certutil -L -d sql:$HOME/.pkg/nssdb |grep "GoAgent" && certutil -d sql:$HOME/.pki/nssdb -D -n "%s"' % ( common_name)
        os.system(cmd_line)
    ...
    def import_ubuntu_system_ca(common_name, certfile):
        ...
        penfile = "/etc/ssl/certs/CA.pem"
        new_certfile = "/user/local/share/ca-certificates/CA.crt"
