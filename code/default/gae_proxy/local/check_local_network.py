...
class CheckNetwork(object):
    def __init__(self, type="IPV4"):
        ...
        self.network_stat="unkown"
        self.last_check_time=0
        self.continue_fail_count=0

        if config.PROXY_ENABLE:
            if config.PROXY_USER:
                self.proxy="%s://%s:%s@%s:%d"%(config.PROXY_TYPE,config.PROXY_USER,config.PROXY_PASSWD,config.PROXY_HOST,config.PROXY_PORT)
            else:
                self.proxy="%s://%s:%d"%(config.PROXY_TYPE,config.PROXY_HOST,config.PROXY_PORT)
        else:
            self.proxy=None
    ...
    def _test_host(self,url):
        try:
            header={"user-agent":"Mozilla/5.0(Windows NT 6.1; WOW64)AppleWebKit/537.36(KHTML,like Gecko) Safari/537.36","accept":"application/json,text/javascript,*/*;q=0.01","accept-encoding":"gzip,deflate,sdch","accept-language":'en-US,en;q=0.8,ja;q=0.6,zh-CN;q=0.4,zh;q=0.2',"connection":"keep-alive"}
...
IPv4.urls=["https://www.microsoft.com","https://www.apple.com","https://code.jquery.com","https://cdn.bootcss.com","https://cdnjs.cloudflare.com"]
IPv4.triger_check_network()

IPv6=CheckNetwork("IPv6")
IPv6.urls=["https://ipv6.vm3.test-ipv6.com","https://[2001:470:1:18::115]","https://ipv6.lookup.test-ipv6.com","https://v6.myip.la"]
