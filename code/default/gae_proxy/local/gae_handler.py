...
def pack_request(method, url, headers, body, timeout):
    ...
    # GAE do not allow set 'Host' header
    ...
    payload += b''.join(b'%s: %s\r\n' % (k,v) for k, v in list(headers.items()) if k not in skip_request_headers)
...
def unpack_response(response):
    try:
        ...
        headers_length, = struct.unpack('!h', data) #本行的‘，’应该是错误？
        data = response.task.read(size = headers_length)
        if not data:
            raise GAE_Exception(600, "get protocol head fail, len:%d" % headers_length)
...
def request_gae_server(headers, body, url, timeout):
    ...
    # process on status code return by http server
    ...
    try:
        ...
        if response.status == 404:
            front.appid_manager.report_not_exist(appid, response.ssl_sock.ip_str)
            response.worker.close("appid not exist:%s" % appid)
            raise GAE_Exception(603, "appid not exist %s" % appid)
        
        if response.status == 503:
            xlog.warning('APPID %r out of Quota, remove it. %s', appid, response.ssl_sock.ip_str)
        ...
        if (b"gws" not in server_type and b"Google Frontend" not in server_type and b"GFE" not in server_type) or response.status == 403 or response.status == 405:

            # some ip can connect, and server type can be gws
            # but can not use as GAE server
            # so we need remove it immediately

            xlog.warn("IP:%s not support GAE, headers:%s status:%d", response.ssl_sock.ip_str, response.headers, response.status)
        ...
        if response.status != 200:
            xlog.warn("GAE %s appid:%s status:%d", response.ssl_sock.ip_str, appid, response.status)
            ...
            if not accept_br_encoding:
                ...
                if response.headers.get(b"Content-Encoding", b"") == b"br":
                    ...
                    if host not in front.config.br_sites:
                        ...
                        front.config.save()
                        front.config.load()
                        xlog.warn("Add %s to br_site", host)
...
def handler(method, host, url, headers, body, wfile, fallback=None):
    ...
    for k, v in list(headers.items()):
        ...
        if k.lower() == b"range":
            req_range = v
            req_range_begin, req_range_end = tuple(x for x in re.search(br'bytes=(\d*)-(\d*)', v).group(1,2))
    ...
    # forece to get content range
    # reduce wait time
    if method == b"GET":
        if req_range_begin and not req_range_end:
            # do not known how many bytes to get, but get from begin position
            req_range_begin = int(req_range_begin)
            headers[b"Range"] = b"bytes=%d-%d" % (req_range_begin, req_range_begin + front.config.AUTORANGE_MAXSIZE -1)
            xlog.debug("change Range %s => %s %s", req_range, headers[b"Range"], url)
            ...
            # not begin and no end
            # do not add range, some host like github do not supprt Range.
            pass
    ...
    if response.status == 206:
        # use org_headers
        # RangeFetch need to know the real range and 
        # 需要分片
        return RangeFetch2(method, url, org_headers, body, response, wfile).run()
    
    response_headers = {}
    # 初始化给客户端的headers
    for key, value in list(response.headers.item()):
        key = key.title()
    ...
    if content_range and b'bytes */' not in content_range:
        start, end, length = tuple(int(x) for x in re.search(br'bytes (\d+)-(\d+)/(\d+)',content_range).group(1,2,3))
    else:
    ...
    def is_text_content_type(content_type):
        ...
        if mct == b'application':
            sct = sct.split(b';', 1)[0]
            if (sct in (b'json', b'javascript', b'x-www-form-urlencoded') or sct.endswith((b'xml', b'script')) or sct.startswith((b'xml', b'rss', b'atom'))):
                return True
        return False
    ...
    if body_length and content_encoding == b"gzip" and response.gps < b"GPS 3.3.2" and is_text_content_type(content_type):
        ...
        if decoded_data1:
            try:
                ...
                if b"deflate" in headers.get(b"Accept-Encoding", b""):
                    ...
                    xlog.info("GAE send ungziped deflate data to browser t:%d s:%d %s %s %s", (time.time() - request_time) * 1000, content_length, method, url, response.task.get_trace())
                    ...
                    while True:
                        ...
                        decoded_data0 = gzip_decompressor.decompress(data)
                        decoded_data1 = deflate_decompressor.decompress(decoded_data0)
                        xlog.info("GAE send ungziped data to browser t:%d s:%d %s %s %s", (time.time() - request_time) * 1000, content_length, method, url, response.task.get_trace())
    ...
    while True:
        ...
        data = response.task.read()
        if not data:
            xlog.warn("get body fail, until:%d %s", body_length - body_sended, url)
            break
        ...
        try:
            ...
            if e_b.args[0] in (errno.ECONNABORTED, errno.EPIPE, errno.ECONNRESET) or 'bad write retry' in repr(e_b):
    ...
    # 完整一次https请求
    appid = response.ssl_sock.host.split(".")[0]
    xlog.info("GAE t:%d s:%d %s %s %s appid:%s", (time.time() - request_time) * 1000, content_length, method, url, response.task.get_trace(), appid)
    return "ok"
...
class RangeFetch2(object):
    ...
    def get_all_buffer_size(self):
        return sum(v for k, v in list(self.all_data_size.item()))
    ...
    def run(self):
        ...
        response_headers = dict((k.title(), v) for k, v in list(self.response.headers.items()))
        content_range = response_headers[b'Content-Range']
        res_begin, res_end, res_length = tuple(int(x) for x in re.search(br'bytes (\d+)-(\d+)/(\d+)', content_range).group(1,2,3))
        ...
        if self.wait_begin == 0 and self.req_end == res_length - 1:
            ...
            response_headers[b'Content-Range'] = b'bytes %d-%d/%d' % (res_begin, self.req_end, res_length)
            response_headers[b'Content-Length'] = bytes(str(self.req_end - res_begin +1), encoding = 'ascii')
            state_code = 206
        ...
        fetch_times = int((data_left_to_fetch + front.config.AUTORANGE_MAXSIZE - 1) / front.config.AUTORANGE_MAXSIZE)
        ...
        ok = "ok"
        while self.keep_running and (front.config.use_ipv6 == "force_ipv6" and check_local_network.IPv6.is_ok() or front.config.use_ipv6 != "force_ipv6" and check_local_network.is_ok()) and self.wait_begin < self.req_end + 1:
            with self.lock:
        ...
        while self.keep_running:
            ...
            if first_response:
                ...
                try:
                    ...
                    xlog.warning('RangeFetch %s request fail:%r', headers[b'Range'], e)
                    continue
            ...
            if response.headers.get(b'Location', None):
                self.url = urllib.parse.urljoin(self.url, response.headers.get(b'Location'))
            ...
            if not content_range:
                xlog.warning('RangeFetch "%s %s" return headers=%r, retry %s-%s', self.method, self.url, response.headers, begin, end)
                continue
            ...
            while True:
                if data_readed >= content_length:
                    ...
                    xlog.debug('RangeFetch [%s] %d%% length:%s range:%s %s %s', response.ssl_sock.ip_str, percent, content_length, content_range, self.url, response.task.get_trace())
                    break
                
                data = response.task.read()
                if not data:
                    xlog.warn("RangeFetch [%s] get body fail, begin:%d %s", response.ssl_sock.ip_str, begin, self.url)
                    break
