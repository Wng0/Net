...
def handler(method, host, path, header, body, wfile, timeout=60):
            ...
            if response:
                if response.status > 600:
                    ...
                    if b"G" not in server_type and b"g" not in server_type and server_type not in google_server_types:
                        xlog.debug("IP:%s host:%s not support GAE, server type:%s status:%d", response.ssl_sock.ip_str, host, server_type, response.status)
    ...
    try:
        ...
        xlog.info("DIRECT t:%d s:%d %d %s %s", (time.time()-time_request)*1000, lenght, response.status, host, path)
        ...
        xlog.exception("DIRECT %s %d %s%s, t:%d send to client except:%r", method, response.status, host, path, (time.time()-time_request)*1000, e)                    